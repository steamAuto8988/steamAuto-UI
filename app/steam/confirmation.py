...
class ConfirmationExecutor:
    ...
    def _set_cookie(self):
        ...
        community_cookie = self._create_cookie('sessionid', self._steamData.sessionId, community_domain)
        self._session.cookies.set(**community_cookie)

        community_cookie = self._create_cookie('steamLoginSecure', self._steamData.steamLoginSecure, community_domain)
        self._session.cookies.set(**community_cookie)

    def __init__(self, steamData: SteamSession) -> None:
        self._session = requests.session()
        self._session.proxies=ProxyConfig.__dict__()
        self._my_steam_id = steamData.steamId
        self._identity_secret = steamData.identitySecret
        self._steamData = steamData
        self._set_cookie()
    ...
    def _send_confirmation(self, confirmation: Confirmation) -> dict:
        ...
        return self._session.get(self.CONF_URL + '/ajaxop', params=params, headers=headers,timeout=10).json()
    ...
    def _fetch_confirmations_page(self) -> requests.Response:
        ...
        response = self._session.get(self.CONF_URL + '/getlist', params=params, headers=headers,timeout=10)
    ...
    def _fetch_confirmation_details_page(self, confirmation: Confirmation) -> str:
        ...
        params = self._create_confirmation_params(tag)
        response = self._session.get(self.CONF_URL + '/details/' + confirmation.data_confid, params=params,timeout=10)
    ...
    def _create_confirmation_params(self, tag_string: str) -> dict:
        ...
        android_id = guard.generate_device_id(str(self._my_steam_id))
        return {'p': android_id,
                'a': self._my_steam_id,
                'k': confirmation_key,
                't': timestamp,
